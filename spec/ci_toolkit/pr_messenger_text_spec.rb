...
describe CiToolkit::PrMessengerText do
  config = CiToolkit::BuildConfig.new({ build_number: 1234, build_url: "https://build.url" })
  build = CiToolkit::Build.new(nil, config)

  it "provides text for new build" do
    sut = described_class.new(build, "https://app.url")
  ...
  it "provides formatted footer" do
    sut = described_class.new(build, "https://app.url")
    expect(sut.footer).to include "https://app.url", build.number.to_s, build.url
  end
